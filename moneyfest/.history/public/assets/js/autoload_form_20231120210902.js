document.addEventListener('DOMContentLoaded', function () {
    ...
    function createAdditionalDropdown() {
        ...
        document.getElementById('additionalDropdown').addEventListener('change', function () {
            ...
            if(selectedKategoriValue!= -1){
                alert('url = '+'/api/produk/'+selectedKategoriValue)
                axios.get('/api/produk/'+selectedKategoriValue)
                    .then(function (response) {
                        // Populate options
                        // response.data.forEach(function (item) {
                        //     const option = document.createElement('option');
                        ...
                        textInputField.value = response; // Replace with the desired logic
                    })
                    .catch(function (error) {
                        console.error('Error fetching data:', error);
                    });
