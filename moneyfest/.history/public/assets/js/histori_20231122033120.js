...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('kategori').addEventListener('change', function () {
    ...
    placeholder = selectedKategoriValue;

    if (selectedKategoriValue == 13 || selectedKategoriValue == 4) {
  ...
  function change_price($id) {
    var uri = '';
    if(placeholder == 13){
      uri = '/api/produk/pemasukan/' + $id;
    }else if (placeholder == 4){
      uri = '/api/produk/pengeluaran/' + $id;
    }
    if ($id != -1) {
      axios.get(uri)
        .then(function (response) {
          response.data.forEach(function (item) {
            ...
            if(placeholder == 13){
              ...
              textInputField.value = item.harga_beli;
            }
