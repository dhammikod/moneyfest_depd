...
class Controller extends BaseController
{
    ...
    public function generate_model()
    {
        ...
        foreach ($result as $item) {
            // Exclude the "total" key from the samples
            $sample = array_diff_key((array)$item, ['total' => 0, 'tahun' => 0, 'bulan' => 0]);

            // Add the sample to the samples array
            $samples[] = $sample;

            // Add the "total" value to the targets array
            $targets[] = $item->total;
        }
        ...
        // Train the model with your samples and targets
    ...
    public function tes_dashboard()
    {
        ...
        foreach ($result as $item) {
            ...
            // Exclude unnecessary keys from the item
            $itemArray = Arr::except($itemArray, ['total', 'tahun', 'bulan']);

            // Cast values to float
            $itemArray = array_map('floatval', $itemArray);

            // Add the modified item array to the samples array
            array_push($samples, $itemArray);

            // Add the "total" value to the targets array
            array_push($targets, (float) $item->total);
        }
