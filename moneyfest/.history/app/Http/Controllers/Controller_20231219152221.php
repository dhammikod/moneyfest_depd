...
class Controller extends BaseController
{
    ...
    public function tes_dashboard()
    {
        ...
        if ($total[2] >= $average_total) {
            ...
            $iklan = $iklan[2] * 110/100;
            $penjualan_produk     = $penjualan_produk[2] * 110/100;
            $royalti = $royalti[2] * 110/100;
            $lisensi = $lisensi[2] * 110/100;
            $donasi = $donasi[2] * 110/100;
            $langganan = $langganan[2] * 110/100;
            $afiliasi = $afiliasi[2] * 110/100;
            $layanan_konsultasi = $layanan_konsultasi[2] * 110/100;
            $penjualan_aset = $penjualan_aset[2] * 110/100;
            $lain_lain = $lain_lain[2] * 110/100;
        } else {
            $prediction = $total[2] * 90 / 100;
            $iklan = $iklan[2] * 90/100;
            $penjualan_produk     = $penjualan_produk[2] * 90/100;
            $royalti = $royalti[2] * 90/100;
            $lisensi = $lisensi[2] * 90/100;
            $donasi = $donasi[2] * 90/100;
            $langganan = $langganan[2] * 90/100;
            $afiliasi = $afiliasi[2] * 90/100;
            $layanan_konsultasi = $layanan_konsultasi[2] * 90/100;
            $penjualan_aset = $penjualan_aset[2] * 90/100;
            $lain_lain = $lain_lain[2] * 90/100;
        }
        ...
        if ($iklanIsZero) {
            $equation = $equation . "iklan: ".strval($iklan[2]);
        }
        if (!$penjualanProdukIsZero) {
            $equation = $equation . "penjualan_produk: ".strval($penjualan_produk[2]);
        }
        if (!$lisensiIsZero) {
            $equation = $equation . "lisensi, ".strval($royalti[2]);
        }
        if (!$royaltiIsZero) {
            $equation = $equation . "royalti, ".strval($lisensi[2]);
        }
        if (!$donasiIsZero) {
            $equation = $equation . "donasi, ".strval($donasi[2]);
        }
        if (!$langgananIsZero) {
            $equation = $equation . "langganan, ".strval($langganan[2]);
        }
        if (!$afiliasiIsZero) {
            $equation = $equation . "afiliasi, ".strval($afiliasi[2]);
        }
        if (!$layananKonsultasiIsZero) {
            $equation = $equation . "layanan konsultasi, ".strval($layanan_konsultasi[2]);
        }
        if (!$penjualanAsetIsZero) {
            $equation = $equation . "penjualan aset, ".strval($penjualan_aset[2]);
        }
        if (!$lainLainIsZero) {
            $equation = $equation . "lain-lain, ".strval($lain_lain[2]);
        }
